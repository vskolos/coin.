...
export default class CurrencyExchangeForm {
  constructor(parent, { accountCurrency }) {
    ...
    allCurrencies()
      ...
      .then((currencies) => {
        ...
        this.accountCurrency.rows = currencies.length
        this.init()
      })
      .catch((error) => handleError(error))
  }

  init(fromChoice = 'BTC', toChoice = 'ETH') {
    const fromChoices = new Choices(this.fromSelect, {
      allowHTML: false,
      searchEnabled: false,
      itemSelectText: '',
    })
    fromChoices.setChoiceByValue(fromChoice)
    ...
    toChoices.setChoiceByValue(toChoice)
